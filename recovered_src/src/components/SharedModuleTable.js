...
export function SearchableCell({ value, resolvedValue, options, onSelect, fontWeight = 400, isRefMode = false, isEmpty = false }) {
  ...
  const isAlias = typeof value === 'string' && (value.startsWith('@') || value.startsWith('='));
...
export default function SharedModuleTable({
  ...
  const partNames = React.useMemo(() => parts.map(p => p.name || ''), [parts]);
  const setupNames = React.useMemo(() => setupItems.map(s => s.name || ''), [setupItems]);
  ...
  const defaultRenderCell = (item, idx, key, isHeader = false) => {
    ...
    return (
      <FormulaInput
        ...
        isHeader={isHeader}
        textAlign={textAlign}
        evaluated={evaluateFormula(item[key], items, spec, isHeader ? {} : parent, 0, setupItems, namedRanges)}
  ...
  const tableBody = (
    <div style={{ overflow: 'visible' }}>
      ...
      {(() => {
          ...
          <table className="bom-table" style={{ ...s.table, border: 'none', tableLayout: 'fixed', minWidth: tableWidth, width: tableWidth }}>
            ...
            <tbody>
              {parent && Object.keys(parent).length > 0 && (() => {
                ...
                const calc = (parent.komp || parent.modul) ? calcBreakdownItem(parent, rowsWithParent, spec, {}) : null;                 const parentRowNumber = (parent._idx !== undefined ? parent._idx : 0) + 1;
                const handleCellClick = (col) => onCellClick && onCellClick(`${col}${parentRowNumber}`);                 const isSel = (col) => col !== 'A' && selectedCoord === `${col}${parentRowNumber}`;
                 const isRef = (col) => col !== 'A' && refCoords.includes(`${col}${parentRowNumber}`);
                const centerCols = ['B', 'C', 'D', 'E', 'F', 'G', 'I', 'J', 'K', 'L', 'M', 'N', 'O', 'P', 'Q', 'R', 'S', 'T', 'U', 'W', 'Y', 'AD', 'AE', 'AF', 'AG', 'AH', 'AI', 'AJ', 'AK', 'AL', 'AM', 'AN', 'AO', 'AP', 'AQ', 'AR'];
              ...
              {items.map((item, rowIdx) => {
                ...
                const handleCellClick = (col) => onCellClick && onCellClick(`${col}${rowNumber}`);                 const isSel = (col) => col !== 'A' && selectedCoord === `${col}${rowNumber}`;
                 const isRef = (col) => col !== 'A' && refCoords.includes(`${col}${rowNumber}`);
                ...
                const componentOptions = item.type === 'Set_up' ? setupNames : partNames;
                ...
                return (
                  ...
                  >                     <td style={cellStyle('A', { background: '#f8fafc', borderRight: '1px solid #e2e8f0', color: '#94a3b8', fontSize: 9, textAlign: 'center', fontWeight: 700 })}>{rowNumber}</td>
                    ...
                    <td style={cellStyle('E')} onMouseDown={(e) => handleMD(e, 'E')}><EditableCell item={item} idx={rowIdx} k="tpk" onUpdateRow={onUpdateRow} isRefMode={isRefMode} /></td>                    <td style={cellStyle('F')} onMouseDown={(e) => handleMD(e, 'F')}>
                       {(() => {
                         // Lookup no dari setupItems berdasarkan nama komponen (untuk semua tipe row)
                         const setupMatch = setupItems.find(s => s.name?.trim() === item.komp?.trim());
                         return <EditableCell item={item} idx={rowIdx} k="no" valueOverride={setupMatch?.no} onUpdateRow={onUpdateRow} isRefMode={isRefMode} />;
                       })()}
                    </td>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                           <SearchableCell
                            ...
                            onSelect={v => {
                              ...
                              if (partMatch) {
                                // Helper: resolve "=varname" dari partsData ke nilai spek aktif                                 if (partMatch.val) {
                                   updates.bid = partMatch.val;
                                 }
                                if (partMatch.bhn) updates.bhn = partMatch.bhn;
                                if (partMatch.t !== undefined) updates.t_bhn = partMatch.t;                                 if (partMatch.code) updates.cat = partMatch.code;
        ...
        const rowsWithParent = parent && Object.keys(parent).length > 0 ? [parent, ...items] : items;
        const calc = item.komp ? calcBreakdownItem(item, rowsWithParent, spec, parent) : null;
                      return (
                        <>
                          {showCNC && <td style={cellStyle('AF', { background: '#f0fdf4', color: '#047857', fontWeight: 600, fontSize: 11, textAlign: 'center' })} onMouseDown={(e) => handleMD(e, 'AF')}>
                            <div style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
