...
if 'Data Validation' in wb_formula.sheetnames:
    ...
    # We will search the entire sheet for "Type Edge" or "cte"
    print("Searching for 'Type Edge' or 'cte'...")
    matches = []
    for r in range(1, 1000):
        for c in range(1, 50):
            val = sheet_v.cell(row=r, column=c).value
            if val == "Type Edge" or val == "cte" or val == "cte" or (isinstance(val, str) and "Type Edge" in val):
                matches.append((r, c, val))
    
    print(f"Found matches: {matches}")
    
    # For each match, let's print the block around it (e.g. 20 rows down, 5 columns wide)
    for r_idx, c_idx, val in matches:
        print(f"\n=== Inspection around Row {r_idx}, Col {c_idx} ({val}) ===")
        # Print column letters A, B, C, D, E... relative to c_idx
