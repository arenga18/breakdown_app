...
async function inspect() {
  try {
    ...
    res.rows.forEach(proj => {
      ...
      if (spec.vals) {
        console.log('  Vals (Spesifikasi Engsel):');
        Object.entries(spec.vals).forEach(([k, v]) => {
          if (k.startsWith('Spesifikasi Engsel')) {
            console.log(`    "${k}": "${v}"`);
          }
        });
      }
      if (spec.aliases) {
        console.log('  Aliases (Spesifikasi Engsel):');
      ...
      if (spec.kodes) {
        console.log('  Kodes (Spesifikasi Engsel):');
        Object.entries(spec.kodes).forEach(([k, v]) => {
          if (k.startsWith('Spesifikasi Engsel')) {
            console.log(`    "${k}": "${v}"`);
          }
        });
